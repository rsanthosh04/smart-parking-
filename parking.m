image = imread('car.jpg'); %To read Cars parked in parking area.
background = imread('bg.jpg'); %To read background image or initial image of parking area
...
%Foreground Detection (back ground)
 thresh=11;
 fr_diff = abs(img-bg);
 for j = 1:width
 for k = 1:height
 ...
 %to convert images with pixel values in different ranges to grayscale images with pixel values normalized between 0 and 1.
 ...
 level=graythresh(sd);%(intensity level of black and white)
 m=imnoise(sd,'gaussian',0,0.025);% apply Gaussian noise(Testing Algorithm Robustness(ability of an algorithm))
 k=wiener2(m,[5,5]);%filtering using Weiner filter(noise reduction and restoration)
 bw=im2bw(k,level);%(to convert grayscale or color images to binary images, where pixel values are either 0 (black) or 1 (white).)
 bw2=imfill(bw,'holes');%(function is used to fill holes in binary images)
 bw3 = bwareaopen(bw2,5000);%(function is used to remove small objects (or connected components))
 labeled = bwlabel(bw3,8);
 %Blob measurements 
 blobMeasurements = regionprops(labeled,'all');%(function is used to compute properties of connected components (regions) in a binary image.)
...
cars=sprintf('[FOREGROUND] , Total space available is %d',totalslot-numberofcars);
 ...
 %IF YES then it divide it no 6 parts as 6 LANEs are there then for each
